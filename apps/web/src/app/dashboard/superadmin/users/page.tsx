...
export default function UserManagementPage() {
  ...
  const loadUsers = async () => {
    try {
      const data = await fetchFromBackend('/users');
      setUsers(data || []);
    } catch (err) {
  ...
  const handleAction = async (e: React.FormEvent) => {
    ...
    const body = editUser ? { name: formData.name } : formData;
            ...
            {users.map(user => (
              <tr key={user.id} className="hover:bg-slate-50/50">
                ...
                <td className="p-4 text-right flex justify-end gap-2">
                  <button 
                    onClick={() => { setEditUser(user); setFormData({ ...formData, name: user.name }); setIsModalOpen(true); }} 
