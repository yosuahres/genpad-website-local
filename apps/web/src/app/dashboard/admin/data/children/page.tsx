...
export default function TemplateCardPage() {
  ...
  const [formData, setFormData] = useState({ child_code: '', name: '',region_id: '' , education_level: '' , academic_year_id: '' , created_by: ''});
  ...
  const loadData = useCallback(async () => {
    try {
      const response = await fetchFromBackend(`/anak-asuh?page=${page}&limit=${PAGE_SIZE}`);
  ...
  const handleAction = async (e: React.FormEvent) => {
    ...
    const url = editItem ? `/anak-asuh/${editItem.id}` : '/anak-asuh';
    ...
    try {
      ...
      setEditItem(null);
      setFormData({ child_code: '', name: '',region_id: '' , education_level: '' , academic_year_id: '' , created_by: ''});
      loadData();
  ...
  return (
    <DashboardLayout roleId={ROLE_IDS.ADMIN}>
              ...
              <tr className="border-b border-slate-200">
                <th colSpan={3} className="py-4 px-6">
                  <div className="flex items-center gap-6">
                    ...
                    <button onClick={() => { setEditItem(selected); setFormData({ child_code: selected.child_code, name: selected.name, region_id: selected.region_id, education_level: selected.education_level, academic_year_id: selected.academic_year_id, created_by: selected.created_by }); setIsModalOpen(true); }} disabled={!selected} className={`flex items-center gap-2 text-xs font-black uppercase tracking-widest ${selected ? "text-slate-600" : "text-slate-300"}`}><Pencil size={18} /> Edit</button>
                    <button onClick={async () => { if(confirm('Delete?')) { await fetchFromBackend(`/anak-asuh/${selected.id}`, {method:'DELETE'}); loadData(); } }} disabled={!selected} className={`flex items-center gap-2 text-xs font-black uppercase tracking-widest ${selected ? "text-slate-600 hover:text-red-600" : "text-slate-300"}`}><Trash2 size={18} /> Delete</button>
                    <button onClick={() => exportToExcel(items, 'Anak Asuh')} className="flex items-center gap-2 text-xs font-black uppercase text-slate-600 tracking-widest"><Download size={18} /> Export</button>
                  </div>
                </th>
              </tr>
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={editItem ? "Edit Anak Asuh" : "Create Anak Asuh"}>
