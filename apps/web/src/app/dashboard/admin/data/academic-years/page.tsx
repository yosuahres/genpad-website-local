...
export default function AcademicYearsPage() {
  ...
  const handleAction = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setEditItem(null);
      setFormData({ year_label: '', is_active: false});
      loadData();
  ...
  return (
    <DashboardLayout roleId={ROLE_IDS.ADMIN}>
      <div className="mb-6 px-4">
        <h2 className="text-xl font-bold text-black mb-8 tracking-tight">Academic Years Management</h2>
              ...
              <tr className="border-b border-slate-200">
                <th colSpan={3} className="py-4 px-6">
                  <div className="flex items-center gap-6">
                    ...
                    <button onClick={() => { setEditItem(selected); setFormData({ year_label: selected.year_label, is_active: selected.is_active }); setIsModalOpen(true); }} disabled={!selected} className={`flex items-center gap-2 text-xs font-black uppercase tracking-widest ${selected ? "text-slate-600" : "text-slate-300"}`}><Pencil size={18} /> Edit</button>
                    <button onClick={async () => { if(confirm('Delete?')) { await fetchFromBackend(`/academic-years/${selected.id}`, {method:'DELETE'}); loadData(); } }} disabled={!selected} className={`flex items-center gap-2 text-xs font-black uppercase tracking-widest ${selected ? "text-slate-600 hover:text-red-600" : "text-slate-300"}`}><Trash2 size={18} /> Delete</button>
                    <button onClick={() => exportToExcel(items, 'Academic Years')} className="flex items-center gap-2 text-xs font-black uppercase text-slate-600 tracking-widest"><Download size={18} /> Export</button>
                  </div>
                </th>
              </tr>
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={editItem ? "Edit Academic Year" : "Create Academic Year"}>
        <form onSubmit={handleAction} className="space-y-4">
          <input className="w-full p-2 border rounded" placeholder="Year Label" value={formData.year_label} onChange={e => setFormData({...formData, year_label: e.target.value})} required />
